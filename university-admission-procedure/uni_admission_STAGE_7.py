...
#  exam_index: physics -> 2, chemistry -> 3, math -> 4, computer science -> 5, admission exam -> 6
...
#  chemistry and physics for the Biotech department (or admission score if better; in each case)
...
faculties_exam_index = {'Biotech': 12, 'Chemistry': 3, 'Engineering': 11, 'Mathematics': 4, 'Physics': 10}
faculties_dict = dict.fromkeys(faculties, [])
waiting_list = set([" ".join(x[0:2]) for x in all_applicants])


#  fac_index: 7 -> first choice, 8 -> second, 9 -> third
def admission_procedure(faculty, exam_index, fac_index):
    global faculties_dict
    global waiting_list
    appl_sorted = sorted(all_applicants, key=lambda x: (-max(x[exam_index], x[6]), x[0], x[1]))
    accepted = []
    ...
    if len(accepted) > 0:
        ...
        if len(faculties_dict[faculty]) > 0:
            ...
            accepted = sorted(accepted, key=lambda x: (-max(x[exam_index], x[6]), x[0], x[1]))
...
def print_accepted(fac, ex_indx):
    ...
    for x in faculties_dict[fac]:
        print(' '.join(x[0:2]), end=' '), print(max(x[ex_indx], x[6]))
...
for key, value in faculties_exam_index.items():
    admission_procedure(key, value, 9)
